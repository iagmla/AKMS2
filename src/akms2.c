...
void usage() {
    printf("AKMS2 v1.0 - by KryptoMagick\n\n");
    printf("Algorithms:\n***********\nlwzf5-cbc        256 bit\n\n");
    printf("Usage:\nakms2 <algorithm> -e <input file> <output file>\n");
    printf("akms2 <algorithm> -d <input file> <output file>\n");
}
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(algorithm, "akms2-cbc") == 0) {
        if (strcmp(mode, encrypt_symbol) == 0) {
            akms2_cbc_encrypt(infile_name, outfile_name, key);
        }
        else if (strcmp(mode, decrypt_symbol) == 0) {
            akms2_cbc_decrypt(infile_name, outfile_name, key);
        }
    }
    printf("\n");
    return 0;
