...
void akms2_ksa(struct akms2_state *state, uint8_t *key, int rounds) {
...
void akms2_encrypt_block(struct akms2_state *state, int rounds) {
...
void akms2_decrypt_block(struct akms2_state *state, int rounds) {
...
void akms2_cbc_encrypt(char *inputfile, char *outputfile, uint8_t *key) {
    struct akms2_state state;
    state.rounds = 64;
    akms2_ksa(&state, key, state.rounds);
    int blocklen = 16;
    int bufsize = 16;
    uint8_t iv[blocklen];
    urandom(iv, blocklen);
    ...
    for (uint32_t b = 0; b < blocks; b++) {
        ...
        akms2_encrypt_block(&state, state.rounds);
...
void akms2_cbc_decrypt(char *inputfile, char *outputfile, uint8_t *key) {
    struct akms2_state state;
    state.rounds = 64;
    akms2_ksa(&state, key, state.rounds);
    int blocklen = 16;
    ...
    for (uint32_t b = 0; b < blocks; b++) {
        ...
        akms2_decrypt_block(&state, state.rounds);
        akms2_cbc_last(&state);
